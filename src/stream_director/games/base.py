...
для промпта, шаблонов-фолбэков. Контракт структурный (Protocol): реализации
ничего не наследуют, но тайпчекер проверяет соответствие. Ядро (директор,
промпты, сервер) работает только через этот контракт и ничего не знает про
конкретную игру.
"""
