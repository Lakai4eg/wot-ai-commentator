...
реагирует ехидными репликами (плашки в OBS + эмоциональный голос OpenAudio
S1-mini на видеокарте) и исполняет команды доверенных зрителей из чата Twitch.
...
Свой голос можно добавить в секции «Голос» панели: WAV 5–12 секунд чистой речи
плюс её текст-транскрипт. «default» — собственный тембр модели без референса.

LLM может ставить в начало реплики эмо-маркер (`(angry)`, `(excited)`,
`(whispering)` и другие) — он меняет интонацию озвучки, но зритель его на плашке
не видит.

Озвучка — модель OpenAudio S1-mini © Fish Audio, лицензия
[CC-BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/); проект
некоммерческий, веса распространяются зеркалом в Releases с сохранением лицензии.
...
`broadcast.py` (WS-оверлей + озвучка S1-mini). Игро-специфичное (память,
